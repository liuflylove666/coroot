...
func anomalyScore(s enrichedStats, severity int) float64 {
	score := 0.0
	// Z-score contribution (max 30 points) — require ≥3σ to reduce false positives
	if s.zScoreMax > 3 {
		score += math.Min(s.zScoreMax*6, 30)
	}
	// Change-point contribution (max 25 points)
	score += s.changeScore * 25
	// Burst ratio contribution (max 15 points) — require ≥5× burst to reduce noise
	if s.burstRatio > 5 {
		score += math.Min((s.burstRatio-5)*3, 15)
	}
...
func detectStatisticalAnomalies(metrics map[string][]*model.MetricValues) []finding {
	...
	for name, values := range metrics {
		...
		for _, mv := range values {
			...
			// Composite anomaly score: z-score + change-point + burst ratio
			score := 0.0
			if s.zScoreMax >= 4 {
				score += math.Min(s.zScoreMax*4, 25)
			}
			score += s.changeScore * 15
			if s.burstRatio > 5 {
				score += math.Min((s.burstRatio-5)*2, 10)
			}

			// Dampen score for metrics with very few data points (startup noise)
			if s.count < 10 {
				score *= 0.3
			}
	...
	for _, a := range anomalies {
		...
		if a.stats.changePoint > 0 {
			cpInfo = fmt.Sprintf(" Change detected at **%s**.", time.Unix(a.stats.changePoint, 0).Format("15:04:05"))
		}

		// Cap statistical anomaly scores well below infrastructure-level findings
		cappedScore := math.Min(a.score, 50)
		results = append(results, finding{
			severity:   sev,
			category:   "Anomaly",
			title:      fmt.Sprintf("Statistical anomaly: `%s`", a.name),
			detail:     fmt.Sprintf("Metric `%s` %s: z-score=**%.1f**, burst ratio=**%.1fx**, change confidence=**%.0f%%**.%s", a.name, a.labels, a.stats.zScoreMax, a.stats.burstRatio, a.stats.changeScore*100, cpInfo),
			evidence:   fmt.Sprintf("%s: min=%.4g, avg=%.4g, max=%.4g, σ=%.4g, z=%.1f, burst=%.1f, change=%.2f, trend=%+.4g/s", a.name, a.stats.min, a.stats.avg, a.stats.max, a.stats.stddev, a.stats.zScoreMax, a.stats.burstRatio, a.stats.changeScore, a.stats.trend),
...
func buildDetailedReport(data RCAData, findings []finding) string {
	...
	sb.WriteString("**Metrics Analysis (Prometheus cache):**\n")
	sb.WriteString("- Z-score analysis — identifies values that deviate significantly from the mean (>2σ)\n")
	sb.WriteString("- CUSUM change-point detection — pinpoints the exact moment when metric behavior shifts\n")
